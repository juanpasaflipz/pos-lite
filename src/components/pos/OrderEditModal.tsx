    ...
    <>
            ...
            {(order.payment_status === 'paid' || order.payment_status === 'completed') &&
              (order.payment_method === 'cash' || order.payment_method === 'split') && (
                <button
                  ...
                  disabled={loading}
                  className="w-full h-11 mt-2 rounded-lg bg-cockpit-green/20 hover:bg-cockpit-green/30 disabled:opacity-40 text-cockpit-in-text font-bold transition-colors inline-flex items-center justify-center gap-1.5 border border-cockpit-green/40"
                >
      ...
      {showCashTip && (
        <CashTipModal
          ...
          existingTip={Number(order.tip) || 0}
