    ...
    <div className="space-y-6">
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
        ...
        <div className="bg-neutral-900 p-6 rounded-lg border border-neutral-800">
          ...
          <p className="text-3xl font-bold text-cockpit-attention-text mt-2">{fmt(cogsData.totals.total_cogs)}</p>
        </div>
        <div className="bg-neutral-900 p-6 rounded-lg border border-neutral-800">
          <p className="text-neutral-400 text-sm">{t('sales.cogs.grossMargin')}</p>
          <p className="text-3xl font-bold text-cockpit-in-text mt-2">{fmt(cogsData.totals.total_margin)}</p>
        </div>
        <div className="bg-neutral-900 p-6 rounded-lg border border-neutral-800">
            ...
            <tbody>
              {cogsData.items.map((item, i) => (
                <tr key={i} className="border-b border-neutral-800 hover:bg-neutral-800/50">
                  ...
                  <td className="px-4 py-3 text-right text-cockpit-attention-text">{fmt(item.cogs)}</td>
                  <td className="px-4 py-3 text-right text-cockpit-in-text">{fmt(item.margin)}</td>
                  <td className="px-4 py-3 text-right">
                    <span className={`font-bold ${item.margin_percent >= 60 ? 'text-cockpit-in-text' : item.margin_percent >= 40 ? 'text-cockpit-attention-text' : 'text-brand-400'}`}>
                      {item.margin_percent}%
                    </span>
                  </td>
                </tr>
