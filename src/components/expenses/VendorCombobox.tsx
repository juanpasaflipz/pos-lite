  ...
  return (
    <div ref={wrapperRef} className="relative">
      <div className="relative">
        ...
        />
        {value.id && (
          <Check size={14} className="absolute right-3 top-1/2 -translate-y-1/2 text-emerald-400" aria-label={t('expenses.vendorLinked')} />
        )}
        {loading && !value.id && (
      ...
      {duplicateWarning && !open && (
        <button
          ...
          className="mt-1 w-full flex items-center gap-2 px-3 py-2 text-left bg-amber-950/40 border border-amber-800/60 rounded-lg text-amber-200 text-xs hover:bg-amber-950/60 transition-colors"
        >
          ...
          {results.map((s, idx) => {
            ...
            return (
              ...
              >
                ...
                {value.id === s.id && <Check size={14} className="text-emerald-400 shrink-0" />}
              </button>
            );
          })}
          {showCreateOption && (
