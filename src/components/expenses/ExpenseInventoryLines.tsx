  ...
  return (
    <div className="space-y-2">
      <div className="flex items-center justify-between">
        ...
        {linesTotal > 0 && (
          <span className={`text-xs ${totalMismatch ? 'text-cockpit-attention-text' : 'text-neutral-500'}`}>
          ...
          <div className="flex items-center justify-between gap-2 px-3 py-2 bg-brand-600/20 border border-brand-600/40 rounded-lg text-sm min-h-[40px]">
            ...
            {canRemove && (
              <button
                type="button"
                onClick={onRemove}
                className="p-1 text-brand-400 hover:text-cockpit-out-text/90 transition-colors shrink-0"
                aria-label="Remove"
              >
                <Trash2 size={14} />
              </button>
