    ...
    <div className="flex flex-col h-full">
              ...
              <div key={item.cart_id} className="bg-neutral-800 rounded-xl p-3 flex items-center gap-3">
                ...
                <div className="flex items-center gap-2 shrink-0">
                  ...
                  <button
                    ...
                    className="w-10 h-10 rounded-full bg-cockpit-red/20 flex items-center justify-center text-cockpit-out-text touch-manipulation ml-1"
                  >
      ...
      {cart.items.length > 0 && (
        <div className="border-t border-neutral-800 bg-neutral-900 px-4 pt-4 pb-4" style={{ paddingBottom: 'calc(env(safe-area-inset-bottom, 0px) + 5rem)' }}>
          ...
          {!isOnline && (
            <p className="text-cockpit-attention-text text-xs text-center mt-2">{t('offline.cashOnly')}</p>
          )}
        </div>
      )}
