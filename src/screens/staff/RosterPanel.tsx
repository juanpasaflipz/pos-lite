...
/**
 * Roster tab for the Staff hub. CRUD over employees + PIN reset.
 * Chrome-less (no header, no back button) — the hub provides those.
 */
export default function RosterPanel() {
  ...
  const roleBadge = (role: RoleType) => {
    switch (role) {
      ...
      case 'manager': return 'bg-cockpit-blue/20 text-cockpit-system-text border border-cockpit-blue';
      case 'kitchen': return 'bg-cockpit-blue/20 text-cockpit-system-text border border-cockpit-blue';
      case 'cashier': return 'bg-cockpit-green/20 text-cockpit-in-text border border-cockpit-green';
  ...
  return (
    <div className="space-y-4">
      <div className="flex flex-wrap items-center justify-between gap-3">
        <div className="text-sm text-neutral-400">
          {limits.employees !== Infinity && (
            <span>{activeCount} / {limits.employees} {t('employees.employeesCount')}</span>
          )}
        </div>
        <button
          onClick={openAdd}
          disabled={isAtLimit('employees', activeCount)}
          className="px-5 py-2.5 bg-brand-600 text-white rounded-lg font-medium hover:bg-brand-700 transition-colors flex items-center gap-2 min-h-[44px] disabled:opacity-50"
        >
          <Plus size={20} />
      ...
      {isAtLimit('employees', activeCount) && (
        <UpgradePrompt message={t('employees.limitReached', { limit: limits.employees })} />
      )}

      {error && (
        <div className="bg-brand-900/30 border border-brand-800 rounded-lg p-4 flex justify-between items-center">
      ...
      {loading ? (
        <div className="space-y-3">
          {[...Array(5)].map((_, i) => (
            <div key={i} className="h-20 bg-neutral-900 rounded-lg border border-neutral-800 animate-pulse" />
          ))}
        </div>
      ) : employees.length === 0 ? (
              ...
              <div className="flex items-center justify-between">
                <div className="flex-1">
                  <div className="flex items-center gap-4 mb-2">
                    ...
                    <span className={`px-3 py-1 rounded-full text-xs font-medium capitalize ${roleBadge(employee.role as RoleType)}`}>
      ...
      {modalMode && (
        <div className="fixed inset-0 bg-black/70 flex items-center justify-center p-4 z-50">
          <div className="bg-neutral-900 rounded-lg border border-neutral-800 shadow-xl max-w-md w-full p-6">
            ...
            <div className="space-y-4">
              <div>
                <label className="block text-sm font-medium text-neutral-300 mb-2">{t('employees.name')}</label>
                <input
                  type="text"
                  value={formData.name}
                ...
                />
                {formErrors.name && <p className="text-brand-400 text-sm mt-1">{formErrors.name}</p>}
              </div>
              ...
              <div>
                <label className="block text-sm font-medium text-neutral-300 mb-2">{t('employees.role')}</label>
                <select
                  value={formData.role}
