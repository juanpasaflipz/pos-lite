...
const KioskPaymentScreen: React.FC = () => {
  ...
  const { lines, total, callName, fulfillmentType, clearCart, setLastOrder } = useKioskCart();
  ...
  const handleCash = async () => {
    ...
    setError(null);
    try {
      const order = await createKioskOrder(auth, orderPayload, 'counter_cash', customerToken, callName, fulfillmentType);
      setLastOrder({
        id: order.id,
  ...
  const handleCard = async () => {
    ...
    try {
      ...
      const order = await createKioskOrder(auth, orderPayload, 'terminal_card', customerToken, callName, fulfillmentType);
