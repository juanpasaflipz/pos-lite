...
- **When a kiosk order reaches the kitchen** is per-tenant: `tenants.kiosk_fire_before_payment` (migration 0106, default **off** — juanbertos is the only tenant opted in, as the pilot; everyone else keeps the pay-first lifecycle until they flip it in Account settings)
  ...
  - **The kiosk fires on the fulfillment tap, not at the end of the flow.** Order of screens is `cart → /fulfillment → /name → pay`; answering "¿para aquí o para llevar?" is what creates the order, and the call-out name is patched on afterwards via `POST /api/kiosk/orders/:id/identify`. `send-to-kitchen` takes `identify_later: true` to skip the name guard. **The two questions are not interchangeable** — fulfillment is a packaging *instruction* (a wrong one is an error the line can't detect), the name is a *label* the KDS already renders honestly as `#order_number` when absent. Defer only the label. If a caller does omit fulfillment, it stores **NULL**, never the `'to_go'` column default; the KDS renders NULL as a neutral "POR CONFIRMAR" pill rather than guessing. `identify` writes only the fields it's given (COALESCE), kiosk-source + unpaid only — the name screen sends the name, and a guest stepping back to change their answer sends fulfillment only, so neither erases the other. Consequence: `/fulfillment` is the point of no return and deliberately the last screen offering a way back to the cart
- **Open-amount lines** (`order_items.is_open_amount`, migration 0109) are the one place in the order path where the **client sets the price**. The cashier taps "Monto abierto" in the POS cart, types a figure on a centavo-first keypad plus an optional concepto, and it rings as an ordinary cart line — so Cobrar → MP/Clip/external terminal, receipts, reports, refunds and CFDI all work untouched. Guard rails: `open_amount: true` is the only thing that unlocks a client price (a `unit_price` on a normal line stays inert), and `resolveOpenAmountLine()` in `server/routes/orders.js` re-validates it (>0, ≤ `OPEN_AMOUNT_MAX`, rounded to centavos, label trimmed to 60 chars)
  - The line is money, not food: `menu_item_id` is NULL, no recipe, no inventory deduction, and `GET /orders/kitchen/active` strips it from every ticket — plus drops the order entirely when open-amount lines are all it has
  - **NULL `menu_item_id` is not the marker — `is_open_amount` is.** `delivery.js` writes NULL for marketplace items it couldn't match to our menu, and those *must* be cooked. Never "simplify" the KDS filter to `menu_item_id IS NULL`; `tests/open-amount.test.ts` catches it
